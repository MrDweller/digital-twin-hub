...
\ArrowheadTitle{remove-digital-twin HTTP/TLS/FORM} %e.g. ServiceDiscovery HTTP/TLS/JSON
...
%  \Large{Service ID: \textit{"\arrowid"}}
  ...
  \begin{abstract}
    ...
    security and Form-data payload encoding variant of the \textbf{remove-digital-twin} service.
  \end{abstract}
  \vspace*{1cm}
...
This document describes the \textbf{remove-digital-twin} service interface, that provides the ability for system operators to remove a digital twin system. 
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|l|l|}
    ...
    Method & DELETE & - \\ \hline
  \end{tabular}
  \caption{Communication and semantics details used for the \textbf{remove-digital-twin}
    service interface}
  \label{tab:comunication_semantics_profile}
\end{table}

This document provides the Interface Design Description IDD to the \textit{remove-digital-twin -- Service Description} document.
For further details about how this service is meant to be used, please consult that document.

The rest of this document describes how to realize the \textbf{remove-digital-twin} service HTTP/TLS/JSON interface in details.
...
\begin{lstlisting}[language=http,label={lst:remove-digital-twin},caption={A \fref{remove-digital-twin-service} invocation.}]
DELETE /digital-twin?address={address}&port={port} HTTP/1.1
...
With these primitives now available, we proceed to define all the types specified in the \textbf{remove-digital-twin} SD document without a direct equivalent among the JSON types.
Concretely, we define the \textbf{remove-digital-twin} SD primitives either as \textit{aliases} or \textit{structs}.
